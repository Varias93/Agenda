...
public class DomicilioDAOSQL implements DomicilioDAO {
	...
	private static final String getD = "SELECT d.idDomicilio FROM domicilio d WHERE d.Calle = ? AND d.Altura = ? AND d.Piso = ? AND d.depto = ?";
	private static final String getId = "SELECT d.* FROM domicilio d WHERE d.idDomicilio = ?";
	private static final String update = "UPDATE domicilio d set calle = ? , altura = ? , piso = ? , depto = ? , localidad = ? where d.idDomicilio = ?";
	...
	public int insert(DomicilioDTO domicilio) {
		...
		try {
			...
			if (statement.executeUpdate() > 0) {
				conexion.commit();
				idDomicilio = get(domicilio.getCalle(), domicilio.getAltura(), domicilio.getPiso(),
						domicilio.getDepto());
			}
